...
bool AcquireMosquittoLib(std::string *error) {
    ...
    if (g_lib_users == 0) {
        ...
        if (rc != MOSQ_ERR_SUCCESS) {
            if (error != nullptr) {
                *error = "mosquitto_lib_init failed: " + std::string(mosquitto_strerror(rc));
            }
            return false;
        }
    }
...
std::string DescribeHostLookup(const std::string &host, int port) {
    ...
    if (rc != 0) {
        std::string message = "resolver failed for '" + host + ":" + port_string + "': " + gai_strerror(rc);
        if (EndsWith(host, ".local")) {
            message += " ('.local' usually needs mDNS; try broker IP or DNS hostname)";
        }
        return message;
    }
    ...
    for (const struct addrinfo *it = results; it != nullptr; it = it->ai_next) {
        ...
        const int name_rc =
            getnameinfo(it->ai_addr, it->ai_addrlen, address, sizeof(address), nullptr, 0, NI_NUMERICHOST);
        if (name_rc == 0) {
    ...
    if (unique_addresses.empty()) {
        return "resolver succeeded for '" + host + ":" + port_string + "' but produced no numeric addresses";
    }

    const std::vector<std::string> addresses(unique_addresses.begin(), unique_addresses.end());
    return "resolver addresses for '" + host + ":" + port_string + "': " + Join(addresses, ", ");
}
} // namespace
...
bool Client::Connect(const std::string &host, int port, const ConnectOptions &options) {
    if (!lib_ready_) {
        return false;
    }
    ...
    mosquitto_reconnect_delay_set(mosq_, options.reconnect_min_seconds, options.reconnect_max_seconds,
                                  options.reconnect_exponential_backoff);

    if (!options.username.empty()) {
        const char *password = options.password.empty() ? nullptr : options.password.c_str();
        const int auth_rc = mosquitto_username_pw_set(mosq_, options.username.c_str(), password);
        if (auth_rc != MOSQ_ERR_SUCCESS) {
            SetLastError("mqtt auth setup failed: " + std::string(mosquitto_strerror(auth_rc)));
            return false;
        }
    }

    if (options.tls_enabled) {
        const char *ca_file = options.tls_ca_file.empty() ? nullptr : options.tls_ca_file.c_str();
        const char *cert_file = options.tls_cert_file.empty() ? nullptr : options.tls_cert_file.c_str();
        const char *key_file = options.tls_key_file.empty() ? nullptr : options.tls_key_file.c_str();

        if ((cert_file == nullptr) != (key_file == nullptr)) {
            SetLastError("mqtt tls setup failed: tls_cert_file and tls_key_file must both be set");
            return false;
        }

        const int tls_rc = mosquitto_tls_set(mosq_, ca_file, nullptr, cert_file, key_file, nullptr);
        if (tls_rc != MOSQ_ERR_SUCCESS) {
            SetLastError("mqtt tls setup failed: " + std::string(mosquitto_strerror(tls_rc)));
            return false;
        }

        const int insecure_rc = mosquitto_tls_insecure_set(mosq_, !options.tls_validate_certificate);
        if (insecure_rc != MOSQ_ERR_SUCCESS) {
            SetLastError("mqtt tls verify setup failed: " + std::string(mosquitto_strerror(insecure_rc)));
            return false;
        }
    }

    const int rc = mosquitto_connect(mosq_, host.c_str(), port, options.keepalive_seconds);
    if (rc != MOSQ_ERR_SUCCESS) {
        std::string error = "connect failed: " + std::string(mosquitto_strerror(rc));
...
#endif
        if (!include_lookup_details && std::string(mosquitto_strerror(rc)) == "Lookup error.") {
...
bool Client::Subscribe(const std::string &topic, int qos) {
    ...
    if (rc != MOSQ_ERR_SUCCESS) {
        SetLastError("subscribe failed topic='" + topic + "': " + std::string(mosquitto_strerror(rc)));
        return false;
    }
    return true;
}

bool Client::Publish(const std::string &topic, const std::string &payload, int qos, bool retain) {
    if (mosq_ == nullptr) {
        SetLastError("publish called before connect");
        return false;
    }
    const int rc = mosquitto_publish(mosq_, nullptr, topic.c_str(), static_cast<int>(payload.size()), payload.c_str(),
                                     qos, retain);
    if (rc != MOSQ_ERR_SUCCESS) {
        SetLastError("publish failed topic='" + topic + "': " + std::string(mosquitto_strerror(rc)));
        return false;
    }
    return true;
}
...
void Client::HandleMessage(struct mosquitto *, void *obj, const struct mosquitto_message *msg) {
    auto *self = static_cast<Client *>(obj);
    ...
    if (msg->payload != nullptr && msg->payloadlen > 0) {
        message.payload.assign(static_cast<const char *>(msg->payload), static_cast<std::size_t>(msg->payloadlen));
    }
