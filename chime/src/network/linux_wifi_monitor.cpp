...
bool WifiStateChanged(const std::optional<WifiState> &before, const WifiState &after) {
    if (!before.has_value()) {
        return true;
    }
    return before->interface_present != after.interface_present || before->operstate != after.operstate ||
           before->carrier != after.carrier;
}

std::optional<WifiState> LinuxWifiMonitor::ReadState(const std::string &interface_name) const {
    if (!oc::util::IsLinux()) {
        return std::nullopt;
    }
