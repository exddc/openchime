...
ApplyManager::ApplyManager(oc::logging::Logger &logger, std::string network_restart_command,
    ...
    : logger_(logger), network_restart_command_(std::move(network_restart_command)),
...
void ApplyManager::RunApplyJob(unsigned long long job_id) {
    ...
    if (!RunCommand(network_restart_command_, &error)) {
        ...
        logger_.Error("webd", "apply job failed id=" + std::to_string(job_id) + " error='" + status_.error + "'");
        return;
    }
