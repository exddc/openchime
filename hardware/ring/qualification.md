...
| Pi rails | 4.75–5.25 V at J8 pin 2 and at TB1 under startup and combined load; 3.3 V within ±5%; no resets or audio/camera faults | Pending measurement |
...
| Buttons | Open GPIO ≥ 1.6 V on the internal pull-up; closed GPIO ≤ 0.9 V through the inline 1 kΩ; no phantom press during boot with the GPIO directives applied | Pending assembly |
...
| Heater | HSW1 switches fully from a 3.3 V GPIO; HTR1 raises the camera carrier above 0 °C at -20 °C ambient; off at boot; duty limit honoured; dew point tracked from U6 | Pending prototype |
...
| Item | Requirement |
| --- | --- |
| Mains | Only J_AC, F0, PSU5, and PSU12 in the partitioned compartment; double-insulated mains wire; PE to PE1, then to the front plate and TB2; installation by a qualified electrician behind a 10 A breaker and a 30 mA residual-current device |
| Harness | Soldered or latching production joints; strain relief at cable entries; no stacked Dupont housings; polarized 20 AWG 5 V feed into J8 pins 2 and 6; R1–R4 and F2 inline under heat-shrink |
...
| Passives | C1/C2 at amplifier VIN; F2 inline in the JD-VCC lead; D1 physically across the strike coil |
...
| E9: heater | Heater part, power, placement, the HSW1 module and its 3.3 V input compatibility, and the dew-point control limits | Thermal soak, condensation results, and measured HSW1 on-state at 0.6 A |
...
| Mechanical selection | Buttons, terminal-block carriers, module mounting, sealing parts, vent, glands, window, and casing depth | Released mechanical BOM and drawings |

Relay wiring is provisional. No relay resistor substitution or driver redesign is approved in this step.

## Scope exclusions

Firmware, automated pin checks, CI regeneration checks for the drawings, infrared illumination, tamper detection, a solid-state strike driver, and any custom circuit board are outside this hardware step. If the transient tests show false button presses, the fallback is a 100 nF capacitor across each button, not a board. The interface specification preserves the requirements needed by later firmware work; the enclosure reserves space and a 12 V feed for the infrared upgrade.
