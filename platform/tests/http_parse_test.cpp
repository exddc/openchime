...
oc::http::HttpReadFn ReaderFromString(const std::string &raw, std::size_t chunk_size = 64) {
...
TEST_SUITE("http_parse") {
    TEST_CASE("parses a GET request and strips the query string") {
        const auto parsed =
            oc::http::ParseHttpRequest("GET /api/v1/config/core?x=1 HTTP/1.1\r\nHost: chime.local\r\n\r\n");
    ...
    TEST_CASE("parses POST body using Content-Length") {
        const auto parsed =
            oc::http::ParseHttpRequest("POST /api/v1/config/core HTTP/1.1\r\nContent-Length: 2\r\n\r\n{}");
        REQUIRE(parsed.success);
    ...
    TEST_CASE("rejects malformed request lines") {
        CHECK_FALSE(oc::http::ParseHttpRequest("GET\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("GET / HTTP/1.1 extra\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("GET / HTTP/2.0\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("get / HTTP/1.1\r\n\r\n").success);
    }

    TEST_CASE("rejects invalid Content-Length") {
        CHECK_FALSE(oc::http::ParseHttpRequest("POST / HTTP/1.1\r\nContent-Length: -1\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("POST / HTTP/1.1\r\nContent-Length: abc\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("POST / HTTP/1.1\r\nContent-Length: 1 2\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("POST / HTTP/1.1\r\nContent-Length: +8\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("POST / HTTP/1.1\r\nContent-Length: 4\r\nContent-Length: 4\r\n\r\nxxxx")
    ...
    TEST_CASE("rejects oversized bodies") {
        const std::string too_big = std::to_string(oc::http::kMaxBodyBytes + 1);
        const auto parsed =
            oc::http::ParseHttpRequest("POST /upload HTTP/1.1\r\nContent-Length: " + too_big + "\r\n\r\n");
        CHECK_FALSE(parsed.success);
        CHECK(parsed.error == "request body too large");
    }

    TEST_CASE("rejects incomplete bodies") {
        const auto parsed = oc::http::ParseHttpRequest("POST / HTTP/1.1\r\nContent-Length: 8\r\n\r\nshort");
        CHECK_FALSE(parsed.success);
    ...
    TEST_CASE("rejects every Transfer-Encoding header") {
        ...
        const auto identity = oc::http::ParseHttpRequest("POST / HTTP/1.1\r\nTransfer-Encoding: identity\r\n\r\n");
    ...
    TEST_CASE("rejects path traversal and encoded separators") {
        CHECK_FALSE(oc::http::ParseHttpRequest("GET /../../etc/passwd HTTP/1.1\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("GET /assets/%2e%2e/secret HTTP/1.1\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("GET /assets/%2fetc/passwd HTTP/1.1\r\n\r\n").success);
        CHECK_FALSE(oc::http::ParseHttpRequest("GET /assets/%00hidden HTTP/1.1\r\n\r\n").success);
    }
    ...
    TEST_CASE("rejects a blank line inside the header block") {
        const auto parsed = oc::http::ParseHttpRequest("GET / HTTP/1.1\r\nHost: chime.local\r\n\nAccept: */*\r\n\r\n");
        CHECK_FALSE(parsed.success);
    ...
    TEST_CASE("streaming reader enforces the same limits") {
        ...
        REQUIRE(oc::http::ReadHttpRequest(ReaderFromString(raw, 7), &request, &error));
        CHECK(request.body == "{}");
        CHECK(request.has_content_type);
        CHECK(request.content_type == "application/json");

        CHECK_FALSE(oc::http::ReadHttpRequest(ReaderFromString("POST / HTTP/1.1\r\nContent-Length: xyz\r\n\r\n"),
                                              &request, &error));
    ...
    TEST_CASE("streaming reader rejects Transfer-Encoding and TE plus Content-Length") {
        oc::http::HttpRequest request;
        std::string error;
        CHECK_FALSE(oc::http::ReadHttpRequest(
    ...
    TEST_CASE("parses Cookie and CSRF headers onto the request") {
        const auto parsed = oc::http::ParseHttpRequest(
            "POST /api/v1/config/core HTTP/1.1\r\nCookie: chime_session=abc; chime_csrf=def\r\n"
            "X-CSRF-Token: def\r\nContent-Length: 2\r\n\r\n{}");
        REQUIRE(parsed.success);
        CHECK(oc::http::RequestHeader(parsed.request, "cookie") == "chime_session=abc; chime_csrf=def");
        CHECK(oc::http::RequestHeader(parsed.request, "X-CSRF-Token") == "def");
        CHECK(parsed.request.body == "{}");
    }
    ...
    TEST_CASE("formats Set-Cookie headers and 401 status text") {
        ...
        const std::string raw = oc::http::FormatHttpResponse(response);
